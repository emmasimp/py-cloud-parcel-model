...
def K01SV(MWAT, T, mass_bin_centre, MSV,n_sv, rhobin, nubin, molwbin,IND1):
    """ Kappa Koehler theory, Petters and Kriedenwies (2007)
   ...
   # ns = c.semi_vol_dict[*mass_semi_vol/molw_sv_bin
  ...
  #  /(c.R*T*RHOAT*Dd))*(nw/(nw+ns))
    ...
    return RH_EQ2, RHOAT, Dw

def SVP_GASES(SV_dict, T, n_sv):
    CSTAR = [SV_dict[key][2] for key in list(SV_dict.keys())[:n_sv]]
    MOLW = [SV_dict[key][0] for key in list(SV_dict.keys())[:n_sv]]
    Del_H_vap = [SV_dict[key][4] for key in list(SV_dict.keys())[:n_sv]]
...
def kappa_recalc(aerosol_mass,condensed_semi_vol ):
    # multiple modes, multiple components #########################################
    solid_aerosol_mass = np.reshape(aerosol_mass,[n.nmodes, n.nbins])
    solid_aerosol_mass_ncomp = np.zeros([n.ncomps, n.nmodes, n.nbins])
    new_aerosol_mass_frac = np.zeros([n.ncomps,n.nmodes, n.nbins])
    
    semi_vol_condensed_mass = np.reshape(condensed_semi_vol,[n.n_sv,n.nmodes, n.nbins])
    ...
    new_aerosol_mole_frac = np.zeros([n.ncomps,n.nmodes, n.nbins])
    sv_mole_frac = np.zeros([n.n_sv,n.nmodes, n.nbins])
    
    molw_bin = np.zeros([n.ncomps,n.nmodes, n.nbins])
    sv_molw_bin = np.zeros([n.n_sv,n.nmodes, n.nbins])
    kappa_bin = np.zeros([n.ncomps,n.nmodes,n.nbins])
    sv_kappa_bin = np.zeros([n.n_sv,n.nmodes,n.nbins])
    rho_bin = np.zeros([n.ncomps,n.nmodes,n.nbins])
    ...
    for m in range(n.nmodes):
          ...
          for j,key in enumerate(list(n.sv_mass_frac.keys())[:n.n_sv]):
              sv_molw_bin[j,m,:] = c.semi_vol_dict[key][0]
              sv_kappa_bin[j,m,:] = c.semi_vol_dict[key][3]
              sv_rho_bin[j,m,:] = c.semi_vol_dict[key][1]
    # mole weighted kappa
    total_aerosol_moles = np.sum(solid_aerosol_mass_ncomp/molw_bin[:9,:,:],axis=0)
    ...
    new_aerosol_vol_frac = np.zeros([n.ncomps,n.nmodes, n.nbins])
    sv_vol_frac = np.zeros([n.n_sv,n.nmodes, n.nbins])
    
    total_aerosol_vol = np.sum(solid_aerosol_mass_ncomp/rho_bin[:9,:,:],axis=0)
