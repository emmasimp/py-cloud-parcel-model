...
@author: mbexkes3
"""
from numpy import zeros, zeros_like, reshape, float32,array
...
def run():
 #   importlib.reload(c)
  ...
  #  from namelist import nbins, nmodes, Mass_frac, P,output_filename, Simulation_type, Dlow, rkm, dt, PRESS1, PRESS2, Temp1, Temp2, Heterogeneous_freezing_criteria, alpha_crit, runtime, T, RH, sig, NAER, D_AER, w
    ...
    sv_mass_frac = {}
    if SV_flag:
        for ind,i in enumerate(semi_vols[:n_sv]):
            
            Mass_frac[i] = [1.0]*nmodes
            sv_mass_frac[i] = [1.0]*nmodes
        ############################################################
 ...
 #   rho_sv = zeros_like(rhobin)
    ...
    GRID = s.setup_grid(rhobin, kappabin, rkm, Dlow, nbins, 
                        nmodes, RH, sig, NAER, D_AER, T, rhoa, k,Mass_frac,
                        c.aerosol_dict,SV_flag)
    ...
    # setup output file
    def setup_output():
        var_names = ['ice_total','liq_total','drop_total','temperature',
                     'pressure','RH','liquid_water_content','ice_water_content']
        ...
        output_file.createDimension('n_sv',n_sv)
        ...
        return output_file
    return nbins, nmodes, simulation_type, PRESS1, PRESS2, w, rhobin, molwbin, nubin, Kappa,Temp1, Temp2, Y, ERROR_FLAG, Y_AER, CDP_CONC_liq, CDP_CONC_ice, CDP_CONC_total,YICE_AER, alpha_crit, ncomps, output, output_ice, dummy2, ACT_DROPS, setup_output, Heterogeneous_freezing_criteria, Mass_frac, Mass_frac_aer, SV_flag, SV_MF, SV_MR, n_sv, semi_vols
          # nbins, nmodes, simulation_type, PRESS1, PRESS2, w, rhobin, molwbin,nubin, Kappa, Temp1, Temp2, Y, ERROR_FLAG, Y_AER, CDP_CONC_liq, CDP_CONC_ice, CDP_CONC_total,YICE_AER, alpha_crit, ncomps, output, output_ice, dummy2, ACT_DROPS, setup_output, Heterogeneous_freezing_criteria, Mass_frac, Mass_frac_aer
