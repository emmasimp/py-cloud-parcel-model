...
def run(Pbar):
    ...
    Heterogeneous_freezing_criteria, Mass_frac, Mass_frac_aer,SV_flag, SV_MF, SV_MR, n_sv, semi_vols] = v.run()
    ...
    def run_sim(Y,time,Y_AER1, YICE):
        ...
        def dy_dt_func(t,Y):
            ...
            if SV_flag:
                MBIN2[-1*n_sv:,:] = np.reshape(Y[INDSV1:INDSV2],[n_sv,nbins*nmodes])
           ...
           #     SV_mass = np.where(SV_mass == 0.0,1e-30,SV_mass)
            #    MBIN2[n.n_sv*-1:,:] = SV_mass
                RH_EQ_SV = f.K01SV(Y[:IND1],Y[ITEMP],MBIN2, 
                                  n_sv, RHOBIN2, NUBIN2, MOLWBIN2)
                RH_EQ = RH_EQ_SV[0]
                RHOAT = RH_EQ_SV[1]
                DW = RH_EQ_SV[2] 
                
                SVP_ORG = f.SVP_GASES(n.semi_vols,Y[ITEMP], n_sv)
                
                RH_ORG = [x*Y[IPRESS]/c.RA/Y[ITEMP] for x in Y[IRH_SV]]
                RH_ORG = [(x/c.aerosol_dict[key][0])*c.R*Y[ITEMP] for x, key in zip(RH_ORG,n.semi_vols[:n_sv])] # just for n_sv keys in dictionary
                RH_ORG = [RH_ORG[x]/SVP_ORG[x] for x in range(n.n_sv)]
                
                dy_dt[INDSV1:INDSV2] = f.SVGROWTHRATE(Y[ITEMP], Y[IPRESS], SVP_ORG,
                                                      RH_ORG, RH_EQ,DW, n_sv, nbins, nmodes,MOLWBIN2)
