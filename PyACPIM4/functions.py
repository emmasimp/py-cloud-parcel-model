...
def Diff(T,P):
    """Diffusion of water vapour in air (cm2·s-1)"""
    T1 = max(T,200)
...
def kk01(MWAT, T, mass_bin_centre, rhobin, kappabin):
    ...
    RHOAT = MWAT/c.rhow+np.sum(mass_bin_centre[:,:]/rhobin[:,:],axis=0)
    RHOAT = (MWAT+np.sum(mass_bin_centre[:,:],axis=0))/RHOAT
    
    Dw = ((MWAT + np.sum(mass_bin_centre[:,:],axis=0))*6/(np.pi*RHOAT))**(1/3) # nearly the same
   # print(mass_bin_centre[:,:])
 ...
 #   Dw = ((MWAT + (mass_bin_centre))*6/(np.pi*RHOAT))**(1/3) # nearly the same
    Dd = ((np.sum(mass_bin_centre[:,:]/rhobin[:,:],axis=0))*6/np.pi)**(1/3) # exactly the same
...
def ACTIVESITES(T, MBIN2, rhobin, nbins, nmodes, ncomps,dt,Mass_frac):
    ...
    NS1 = np.zeros(len(c.aerosol_dict.keys()))
...
def kk01SV(MWAT, T, mass_bin_centre, rhobin, kappabin):
    """ Kappa Koehler theory, Petters and Kriedenwies (2007)
        
    """
    rhobin2 = np.ones(np.shape(rhobin))*c.semi_vol_dict['SV01'][1]
  #  molwbin2 = np.ones(np.shape(molwbin))*c.semi_vol_dict['SV01'][0]
  
    RHOAT = MWAT/c.rhow+(mass_bin_centre/rhobin2)
    RHOAT = (MWAT+(mass_bin_centre))/RHOAT
   ...
   # RHOAT = c.rhow
    ...
    Dw = ((MWAT + (mass_bin_centre))*6/(np.pi*RHOAT))**(1/3) # nearly the same
    Dd = ((mass_bin_centre*6)/(rhobin2*np.pi))**(1/3) # exactly the same
    #KAPPA = (mass_bin_centre/rhobin*kappabin)/(mass_bin_centre/rhobin)
    KAPPA = kappabin
    sigma = surface_tension(T) # 
    
    RH_EQ = ((Dw**3-Dd**3)/(Dw**3-Dd**3*(1-KAPPA))*
                 np.exp((4*sigma*c.semi_vol_dict['SV01'][0])/(c.R*T*c.semi_vol_dict['SV01'][1]*Dw)))
...
def K01SV(MWAT, T, mass_bin_centre,n_sv, rhobin, nubin, molwbin):
    """ Kappa Koehler theory, Petters and Kriedenwies (2007)
   ...
   # print(mass_bin_centre[:,:])
    ...
    RH_EQ = (np.exp(4*molwbin[-1*n_sv:,:]*sigma/(c.R*T*np.tile(RHOAT*Dw,(n_sv,1))))*
            mass_bin_centre[-1*n_sv:,:]/molwbin[-1*n_sv:,:]*nubin[-1*n_sv:,:]*
             np.tile((1/(nw+np.sum(mass_bin_centre[:,:]/molwbin[:,:]*nubin[:,:],axis=0))),(n_sv,1)))

    return RH_EQ, RHOAT, Dw

def K01(MWAT, T, mass_bin_centre,n_sv, rhobin, nubin, molwbin):
    """ Kappa Koehler theory, Petters and Kriedenwies (2007)
   ...
   # print(mass_bin_centre[:,:])
    ...
    RH_EQ = (np.exp(4*c.mw*sigma/(c.R*T*RHOAT*Dw))*
             (nw/(nw+np.sum(mass_bin_centre[:,:]/molwbin[:,:]*nubin[:,:]))))
             #np.sum(mass_bin_centre[:,:]/molwbin[:,:]*nubin[:,:],axis=0)*
             #(1/(nw+np.sum(mass_bin_centre[:,:]/molwbin[:,:]*nubin[:,:],axis=0))))

    return RH_EQ, RHOAT, Dw

def SVP_GASES(SV_dict, T, n_sv):
 #   CSTAR = [SV_dict[key][2] for key in list(SV_dict.keys())[:n_sv]]
    CSTAR = [c.aerosol_dict[key][4] for key in SV_dict[:n_sv]]
    MOLW = [c.aerosol_dict[key][0] for key in SV_dict[:n_sv]]
    Del_H_vap = [c.aerosol_dict[key][5] for key in SV_dict[:n_sv]]
    PI = [((10**CSTAR[i])*298.15*c.R)/MOLW[i]*1e-9 for i in range(n_sv)]
    
    SVP_GASES = [PI[i]*np.exp(Del_H_vap[i]/c.R*(1/298.15-1/T)) for i in range(n_sv)]
    
    return SVP_GASES
    
    
def SVGROWTHRATE(T, P, SVP_ORG, RH, RH_EQ,DIAM, n_sv,nbins,nmodes,molwbin):
     

   
    sv_growthrate = np.zeros([n_sv, nbins*nmodes])
    RAD = DIAM/2
    D1 = Diff(T,P)
    
    svp_org = np.ones([n_sv,nbins*nmodes])
    rh = np.ones([n_sv,nbins*nmodes])
    for i in range(n_sv):
        svp_org[i,:] = svp_org[i,:]*SVP_ORG[i]
        rh[i,:] = rh[i,:]*RH[i]
  
    sv_growthrate[:,:] = (4*np.pi*np.reshape(np.tile(RAD,(n_sv)),[n_sv,nbins*nmodes])*D1*
                          (rh[:,:]-RH_EQ[:,:])*
                           svp_org[:,:]/(c.R/molwbin[-1*n_sv:,:]*T))
    
    return np.reshape(sv_growthrate,[nbins*nmodes*n_sv])
    
    
   
def kappa_recalc(aerosol_mass,condensed_semi_vol,n_sv, nmodes, nbins, Mass_frac_aer, ncomps,semi_vols ):
    # multiple modes, multiple components #########################################
    #print(aerosol_mass,condensed_semi_vol,n_sv, nmodes, nbins, Mass_frac_aer, ncomps)
    solid_aerosol_mass = np.reshape(aerosol_mass,[nmodes, nbins])
    solid_aerosol_mass_ncomp = np.zeros([ncomps, nmodes, nbins])
    new_aerosol_mass_frac = np.zeros([ncomps,nmodes, nbins])
    semi_vol_condensed_mass = np.reshape(condensed_semi_vol,[n_sv,nmodes, nbins])
    sv_mass_frac = np.zeros([n_sv,nmodes, nbins])
    
    for mode in range(nmodes):
        for i,key in enumerate(list(Mass_frac_aer.keys())[:ncomps]):
            solid_aerosol_mass_ncomp[i,mode,:] = solid_aerosol_mass[mode,:]*Mass_frac_aer[key][mode]
            
 #   print(condensed_semi_vol)
    for mode in range(nmodes):
    ...
    new_aerosol_mole_frac = np.zeros([ncomps,nmodes, nbins])
    sv_mole_frac = np.zeros([n_sv,nmodes, nbins])
    
    molw_bin = np.zeros([ncomps,nmodes, nbins])
    sv_molw_bin = np.zeros([n_sv,nmodes, nbins])
    kappa_bin = np.zeros([ncomps,nmodes,nbins])
    sv_kappa_bin = np.zeros([n_sv,nmodes,nbins])
    rho_bin = np.zeros([ncomps,nmodes,nbins])
    sv_rho_bin = np.zeros([n_sv,nmodes,nbins])
    ...
    for m in range(nmodes):
          ...
          for j,key in enumerate(semi_vols[:n_sv]):
              sv_molw_bin[j,m,:] = c.aerosol_dict[key][0]
              sv_kappa_bin[j,m,:] = c.aerosol_dict[key][3]
              sv_rho_bin[j,m,:] = c.aerosol_dict[key][1]

    # mole weighted kappa
    total_aerosol_moles = np.sum(solid_aerosol_mass_ncomp/molw_bin[:ncomps,:,:],axis=0)
 
    total_sv_moles = np.sum(semi_vol_condensed_mass/sv_molw_bin,axis=0)
    
    

    for mode in range(nmodes):
        for i in range(ncomps):
            new_aerosol_mole_frac[i,mode,:] = ((solid_aerosol_mass_ncomp[i,mode,:]/molw_bin[i,mode,:])/
                                           (total_aerosol_moles[mode,:]+total_sv_moles[mode,:]))
        for j in range(n_sv):
            sv_mole_frac[j,mode,:] =( (semi_vol_condensed_mass[j,mode,:]/sv_molw_bin[j,mode,:])/
                                     (total_aerosol_moles[mode,:]+total_sv_moles[mode,:]))
    
    kappa_new = np.sum(new_aerosol_mole_frac*kappa_bin,axis=0) # sum over all components
    sv_kappa_new = np.sum(sv_mole_frac*sv_kappa_bin,axis=0) # sum over all components
    
    total_new_kappa = kappa_new + sv_kappa_new
    
    # volume weighted kappa
    new_aerosol_vol_frac = np.zeros([ncomps,nmodes, nbins])
    sv_vol_frac = np.zeros([n_sv,nmodes, nbins])
    
    total_aerosol_vol = np.sum(solid_aerosol_mass_ncomp/rho_bin[:ncomps,:,:],axis=0)
    total_sv_vol = np.sum(semi_vol_condensed_mass/sv_rho_bin,axis=0)
    
    for mode in range(nmodes):
        for i in range(ncomps):
            new_aerosol_vol_frac[i,mode,:] = ((solid_aerosol_mass_ncomp[i,mode,:]/rho_bin[i,mode,:])/
                                           (total_aerosol_vol[mode,:]+total_sv_vol[mode,:]))
        for j in range(n_sv):
            sv_vol_frac[j,mode,:] =( (semi_vol_condensed_mass[j,mode,:]/sv_rho_bin[j,mode,:])/
                                     (total_aerosol_vol[mode,:]+total_sv_vol[mode,:]))
...
def write_output_file(output_file,output,output_ice,IND1,IND2,ITEMP,IPRESS,IRH,
                      IRH_SV,CDP_CONC_total,SV_flag,INDSV1,INDSV2,nmodes,nbins,n_sv,idx,ACT_DROPS):
